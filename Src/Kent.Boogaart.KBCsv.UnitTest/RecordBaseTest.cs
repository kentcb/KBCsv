...
namespace Kent.Boogaart.KBCsv.UnitTest
{
    ...
    public sealed class RecordBaseTest
    {
        ...
        {
            _record = new ConcreteRecordBase(new string[] { "value1", "value2", "value3", "value4" });
            Assert.Equal("value1", _record.GetValueOrNull(0));
            Assert.Equal("value2", _record.GetValueOrNull(1));
            Assert.Equal("value3", _record.GetValueOrNull(2));
            Assert.Equal("value4", _record.GetValueOrNull(3));
            Assert.Null(_record.GetValueOrNull(-1));
            Assert.Null(_record.GetValueOrNull(4));
            Assert.Null(_record.GetValueOrNull(int.MaxValue));
            Assert.Null(_record.GetValueOrNull(int.MinValue));
        }
