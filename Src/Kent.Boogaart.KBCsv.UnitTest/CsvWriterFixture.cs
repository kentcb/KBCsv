namespace Kent.Boogaart.KBCsv.UnitTest
{
    ...
    public sealed class CsvWriterFixture
    {
        ...
        [Fact]
        public void value_separator_throws_if_white_space()
        {
            using (var writer = new CsvWriter(new MemoryStream()))
            {
                var ex = Assert.Throws<ArgumentException>(() => writer.ValueSeparator = ' ');
                Assert.Equal("Value separator and delimiter cannot be white space.", ex.Message);

                ex = Assert.Throws<ArgumentException>(() => writer.ValueSeparator = '\t');
                Assert.Equal("Value separator and delimiter cannot be white space.", ex.Message);
            }
        }
        ...
        [Fact]
        public void value_delimiter_throws_if_white_space()
        {
            using (var writer = new CsvWriter(new MemoryStream()))
            {
                var ex = Assert.Throws<ArgumentException>(() => writer.ValueDelimiter = ' ');
                Assert.Equal("Value separator and delimiter cannot be white space.", ex.Message);

                ex = Assert.Throws<ArgumentException>(() => writer.ValueDelimiter = '\t');
                Assert.Equal("Value separator and delimiter cannot be white space.", ex.Message);
            }
        }
