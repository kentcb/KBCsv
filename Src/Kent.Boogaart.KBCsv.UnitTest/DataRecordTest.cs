...
namespace Kent.Boogaart.KBCsv.UnitTest
{
    ...
    public sealed class DataRecordTest
    {
        ...
        [Fact]
        public void TestColumnNameIndexer()
        {
            var dataRecord = new DataRecord(null);
            var ex = Assert.Throws<InvalidOperationException>(() => dataRecord["anything"]);
        ...
        {
            _data = new DataRecord(new HeaderRecord(new string[] { "Name", "Age", "Gender", "Relationship Status" }), new string[] { "Kent", "25", "M" }, true);
            Assert.Equal("Kent", _data.GetValueOrNull("Name"));
            Assert.Equal("25", _data.GetValueOrNull("Age"));
            Assert.Equal("M", _data.GetValueOrNull("Gender"));
            Assert.Null(_data.GetValueOrNull("Relationship Status"));
            Assert.Null(_data.GetValueOrNull("Some non-existant column"));
        }
    }
}
