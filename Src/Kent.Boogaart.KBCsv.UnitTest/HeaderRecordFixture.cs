namespace Kent.Boogaart.KBCsv.UnitTest
{
    ...
    public sealed class HeaderRecordFixture
    {
        ...
        [Fact]
        public void public_constructor_throws_values_is_null()
        {
            Assert.Throws<ArgumentNullException>(() => new HeaderRecord((IEnumerable<string>)null, false));
        }

        [Fact]
        public void public_constructor_throws_if_any_value_is_null()
        {
            Assert.Throws<ArgumentException>(() => new HeaderRecord((IEnumerable<string>)new string[] { "column1", null, "column3" }));
        }

        [Fact]
        public void public_constructor_assigns_empty_values_if_columns_array_is_empty()
        {
            var header = new HeaderRecord((IEnumerable<string>)new string[0]);
        ...
        {
            var header = new HeaderRecord((IEnumerable<string>)new string[] { "Name", "Age", "Gender" });
        ...
        [Fact]
        public void public_constructor_always_creates_new_list()
        {
            var values = new string[] { "one", "two" }.ToList();
            var header = new HeaderRecord(values, false);
            header.Add("three");
            Assert.False(values.Contains("three"));
        }

        [Fact]
        public void internal_constructor_results_in_read_only_record()
        {
