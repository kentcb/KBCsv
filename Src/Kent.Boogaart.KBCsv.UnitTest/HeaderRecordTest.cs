...
namespace Kent.Boogaart.KBCsv.UnitTest
{
    ...
    public sealed class HeaderRecordTest
    {
        ...
        {
            _header = new HeaderRecord(new string[] { "Name", "Age", "Gender" }, true);
            Assert.Equal(0, _header.IndexOf("Name"));
            Assert.Equal(1, _header.IndexOf("Age"));
            Assert.Equal(2, _header.IndexOf("Gender"));
            Assert.Equal(-1, _header.IndexOf("name"));
            Assert.Equal(-1, _header.IndexOf("some non-existant column"));
        }
    }
}