namespace Kent.Boogaart.KBCsv.UnitTest.Internal
{
    using System;
    using System.IO;
    using System.Text;
...
value4,   value5   ,	value6	";
        ...
        [Fact]
        public void value_delimiter_throws_if_set_to_space()
        {
            var parser = this.CreateParserFromString(string.Empty);
            var ex = Assert.Throws<ArgumentException>(() => parser.ValueDelimiter = ' ');
        ...
        [Fact]
        public void value_separator_throws_if_set_to_space()
        {
            var parser = this.CreateParserFromString(string.Empty);
            var ex = Assert.Throws<ArgumentException>(() => parser.ValueSeparator = ' ');
            Assert.Equal("Space is not a valid value separator or delimiter.", ex.Message);
...
fourth";
        ...
        {
            var headerRecord = new HeaderRecord(new string[] { "Name", "Age" }, true);
            var csv = @"Kent,33
Belinda,34
Tempany,8
Xak,0";
...
over two lines'", records[0][3]);
        ...
        public void parse_records_can_handle_large_complex_data_set()
        {
            ...
            for (var record = 0; record < 1500; ++record)
            {
                var fields = random.Next(1, 500);
